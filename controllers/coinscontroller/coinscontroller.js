...
import { getTeamPurchased, currencyValues } from '../../service/refferralSystem/refferral.js'
...
export const addCoinsOnProductBuy = async (req, res) => {
  try {
    ...
    const precentTeam = Object.values(teamData.data);

    if (userDetails[0].team != null) putCoinsTeam(JSON.parse(userDetails[0].team), coins, precentTeam);
    const queryAddCoins = `UPDATE coins SET value=value+? WHERE user_id=?`;
    let precentageCoin = (precentTeam[1] * coins) / 100;
    const value = [precentageCoin, userId];
...
const putCoinsTeam = async (teams, coins, precentTeam) => {

  const currency = await currencyValues();

  // const totalAmount=coins*currency;
  ...
  for (let i = n - 1; i >= x; i--) {
    const queryCheckUser = `SELECT user_id FROM team_referral`;
    const valueCheck = [teams[i]];
    const checkUser = await queryPromise(queryCheckUser, valueCheck);
    if (checkUser.length == 0) continue;
    const queryAddCoins = `UPDATE coins SET value=value+? WHERE user_id=?`;
    let coin = (coins * precentTeam[k++]) / 100;

    const value = [coin, checkUser[0].user_id];
    await queryPromise(queryAddCoins, value);
  }
}
